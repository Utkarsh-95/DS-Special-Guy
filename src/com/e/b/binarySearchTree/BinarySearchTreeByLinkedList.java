    ...
    public BinaryNode deleteNodeOfBST(BinaryNode root, int value) {
        ...
        if (value < root.getData()) {
            ...
            if (root.getLeft() != null && root.getRight() != null) { // if nodeToBeDeleted have both children
                ...
                root.setData(minNodeForRight.getData()); // Replacing current node with minimum node from right subtree
                root.setRight(deleteNodeOfBST(root.getRight(), minNodeForRight.getData()));  // Deleting minimum node from right now
